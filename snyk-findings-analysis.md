...
- **Reasoning:** The executed binary is `exec.LookPath(os.Args[0])` (`main.go:356`), and its arguments are what `tui.Run(os.Args[1:])` returns (`main.go:346`) after `expandTilde` (`main.go:302-319`, called at `main.go:354`). The branch runs inside `init()` only when stdout is a terminal and no subcommand, or only `analyze`, was given (`main.go:345`). The sink at `main.go:360-361` passes the binary and its argument slice to `syscall.Exec`, which hands each element to `execve` as a separate argument, and no shell parses it; the comment at `main.go:351-353` records that shell invocation was removed. Only the operator controls `argv[0]`, `PATH` and the wizard input, and the operator can already run any program, so the CWE-78 premise of untrusted data reaching a shell command does not hold. Criterion: false positive, trusted-source and not-the-weakness clauses.
...
- **Summary:** The Pusher detector verifies a candidate key by posting a fixed test event to Pusher's API with a signed query string. Snyk names the constant request body as the source; separately, the HTTP client that sends the request follows any redirect the response names.
- **Verdict:** False positive
- **Reasoning:** The flow Snyk reports runs from the constant body (`pkg/detectors/pusherchannelkey/pusherchannelkey.go:115-116`) through the request signature (`pkg/detectors/pusherchannelkey/pusherchannelkey.go:132`) into the URL of the request built at `pkg/detectors/pusherchannelkey/pusherchannelkey.go:136`. That URL goes to the fixed host `https://api-ap1.pusher.com/apps/` (`pkg/detectors/pusherchannelkey/pusherchannelkey.go:134`). The app ID and key interpolated into its path and query come from scanned content but must match `[0-9]{7}` and `[a-z0-9]{20}` (`pkg/detectors/pusherchannelkey/pusherchannelkey.go:35-36`), and the signature is hex-encoded, so no value on this flow can change the scheme, host or port of the flagged request.

  A separate weakness sits at the same call, and Snyk does not report it. The request is sent on the package client `common.SaneHttpClient()` (`pkg/detectors/pusherchannelkey/pusherchannelkey.go:32`, `pkg/common/http.go:259-264`), which sets no `CheckRedirect` and blocks no local address, so `client.Do` (`pkg/detectors/pusherchannelkey/pusherchannelkey.go:142`) follows up to ten redirects to wherever each response's `Location` header points. Under this report's trust model a provider response, including a URL it supplies for a follow-up request, is untrusted: whoever controls the TLS-authenticated api-ap1.pusher.com response can turn a 302 or 303 into a GET, or a 307 or 308 into a repeated POST, to loopback, link-local metadata or other internal addresses. That request is blind, because the outcome is reduced to a verified flag or an error (`pkg/detectors/pusherchannelkey/pusherchannelkey.go:153-160`), and `SECURITY.md:4` and `SECURITY.md:11-14` class blind SSRF as hardening. The redirect target never reaches the flagged `http.NewRequestWithContext` call, because `net/http` builds each follow-up request itself; the weakness would meet the taint clause as a finding of its own, but it does not make the value at this result's sink attacker-controlled. Recommendation, outside the Blitzy prompt, which covers only true positives: send this request on a client that follows no redirects and refuses local addresses, such as `detectors.DetectorHttpClientWithNoLocalAddresses` (`pkg/detectors/http.go:40-45`), so that a redirect comes back as its 3xx status and ends in the `default` branch as a verification error (`pkg/detectors/pusherchannelkey/pusherchannelkey.go:158-159`). Criterion for the flow Snyk reports: false positive, trusted-source clause: constant and regex-constrained values land in a fixed host.
...
The prompt below covers the 12 true positives, #53–#63 and #65, in three changes, listed in the table. It is a proposal for a separate Blitzy run: it has not been run, and the only change to the repository is the addition of this report; no source, test, configuration or A–C change has been made. Each change first states the change planned in the Technical Specification §0.4.6. Changes A and B then add steps beyond it, labelled "Added hardening", because the planned change alone leaves residuals; each added step names the residual it closes or, for the userinfo check in A, its reason. Change C adds none. The false positives need no change, and #27 is excluded until its manual review returns a verdict. One false positive carries a separate recommendation: the Pusher request under #64 follows provider redirects unchecked, a weakness Snyk does not report, and the #64 entry recommends sending that request on a client that follows no redirects and refuses local addresses; the prompt leaves it out because it addresses only the true positives.

| Change | Results | File changed | Fix |
| --- | --- | --- | --- |
| A, ngrok pagination origin | #53–#60 | `pkg/analyzer/analyzers/ngrok/requests.go`, `pkg/analyzer/analyzers/ngrok/ngrok.go` | **Planned change (Technical Specification §0.4.6):** in `fetchResources` (`pkg/analyzer/analyzers/ngrok/requests.go:190-209`), return a fixed-message error before any request is sent unless the URL parses, its scheme is `https` and its host equals the host of `ngrokAPIBaseURL` (`pkg/analyzer/analyzers/ngrok/requests.go:14`). `makeAPIRequest` and the six loops stay as they are; every loop passes `res.NextPageURI` to `fetchResources` (`pkg/analyzer/analyzers/ngrok/requests.go:97` and the five matching lines), so the one check covers the pagination URLs of all eight results. **Added hardening:** the check also refuses a URL that carries userinfo: no genuine pagination URL needs it, so only plain API URLs are accepted. The client from `analyzers.NewAnalyzeClient` sets no `CheckRedirect` (`pkg/analyzer/analyzers/client.go:35-51`), so `net/http` follows redirects and an allowed URL could still answer with a redirect off the ngrok API; in `AnalyzePermissions` (`pkg/analyzer/analyzers/ngrok/ngrok.go:72`), pass `analyzers.NewAnalyzeClient` an option through its existing parameter (`pkg/analyzer/analyzers/client.go:47-49`) that sets `CheckRedirect` to apply the same test to every redirect target, so the check and the redirect option together cover all eight results, redirects included. `fetchResources` and `determineAccountType` (`pkg/analyzer/analyzers/ngrok/requests.go:24-57`) return that error unwrapped, because `net/http` quotes the rejected `Location` in the error it wraps around it. |
| B, Docker local addresses | #61–#63 | `pkg/detectors/docker/docker_auth_config.go` | **Planned change (Technical Specification §0.4.6):** in `FromData`, replace the `common.SaneHttpClient()` fallback (`pkg/detectors/docker/docker_auth_config.go:127-131`) with `detectors.DetectorHttpClientWithNoLocalAddresses` (`pkg/detectors/http.go:40-45`) and drop the `pkg/common` import (`pkg/detectors/docker/docker_auth_config.go:16`), whose only use is that fallback; keep the injected `s.client` path. `verifyMatch` sends both requests on that client (`pkg/detectors/docker/docker_auth_config.go:165`, `pkg/detectors/docker/docker_auth_config.go:218`). **Added hardening:** the shared client leaves two gaps: its dial guard checks the resolved addresses (`pkg/detectors/http.go:142-149`) but then dials the hostname, which is resolved again (`pkg/detectors/http.go:151`), so DNS rebinding passes it, and through an environment proxy (`pkg/detectors/http.go:89`) it checks the proxy's address, not the registry's. The fallback is therefore a Docker-guarded copy of that client's value rather than the client itself, as `detectors.NewClientWithDedup` already copies a client (`pkg/detectors/http.go:264-271`): the copy keeps its timeout and its no-redirect policy, and only the copy's transport is replaced, by one defined in `pkg/detectors/docker/docker_auth_config.go` that resolves the registry once, refuses any loopback, link-local, private or unspecified answer with `detectors.ErrNoLocalIP` (`pkg/detectors/http.go:101-109`), dials only an address it checked, and refuses to send a verification request through a proxy. The shared client is never mutated, and `pkg/detectors/http.go` stays untouched. Residual: addresses outside those classes stay reachable, for example 100.64.0.0/10, which includes the Alibaba Cloud metadata address 100.100.100.200, as they do for the planned change and every detector on the shared client. |
...
- What components are affected?
  - Change A: pkg/analyzer/analyzers/ngrok/requests.go, function fetchResources (lines 190-209), plus a new unexported origin error and origin check. Added hardening: function determineAccountType (lines 24-57) in the same file, and pkg/analyzer/analyzers/ngrok/ngrok.go, function AnalyzePermissions (client creation at line 72), plus a new unexported redirect option.
  - Change B: pkg/detectors/docker/docker_auth_config.go, function FromData (client fallback at lines 127-131, pkg/common import at line 16). Added hardening: a package-level guarded copy of detectors.DetectorHttpClientWithNoLocalAddresses and the unexported constructor of its transport, in the same file.
...
- What must remain untouched?
  ...
  - roundtripper.WithInsecureTLS, pkg/detectors/http.go and its shared detector clients, and pkg/analyzer/analyzers/client.go; change A reaches the analyzer client only through the existing option parameter of NewAnalyzeClient.
...
- How should fixes be implemented?
  - Change A:
    - Planned change (Technical Specification §0.4.6):
      - In pkg/analyzer/analyzers/ngrok/requests.go, add an unexported origin error with a fixed message that names neither the key nor any URL, and an unexported origin check that accepts a URL only when it parses with net/url, its scheme is https, and its host equals the host of ngrokAPIBaseURL (pkg/analyzer/analyzers/ngrok/requests.go:14).
      - In fetchResources, return the origin error, before any request is sent, when the URL fails the origin check.
      - Leave makeAPIRequest and the six pagination loops unchanged. The loops all pass res.NextPageURI to fetchResources, so the one origin check covers the pagination URLs of #53-#60.
    - Added hardening beyond the planned change, to close the residuals it leaves and to accept only plain API URLs:
      - Make the origin check also refuse a URL that carries userinfo. No genuine pagination URL needs userinfo, so only plain API URLs are accepted.
      - In pkg/analyzer/analyzers/ngrok/ngrok.go, add an unexported client option that sets CheckRedirect to return the origin error when a redirect target fails the origin check, and to stop after 10 redirects as Go's default policy does. In AnalyzePermissions, pass it to analyzers.NewAnalyzeClient (pkg/analyzer/analyzers/ngrok/ngrok.go:72) through the existing option parameter, which is applied after any logging wrapper (pkg/analyzer/analyzers/client.go:35-51). NewAnalyzeClient sets no redirect policy, so without the option net/http follows a redirect from an allowed URL to wherever its Location header points; with it, every request the analyzer sends refuses a redirect off https://api.ngrok.com, including the POST in determineAccountType (pkg/analyzer/analyzers/ngrok/requests.go:32-33). The origin check and the redirect option together then cover #53-#60, redirects included.
      - In fetchResources and determineAccountType, when the error from makeAPIRequest wraps the origin error (errors.Is), return the origin error itself. net/http wraps a CheckRedirect error in a url.Error that quotes the Location value, so returning it unwrapped keeps the rejected URL out of the analyzer's error output; the key is in neither message.
  - Change B:
    - Planned change (Technical Specification §0.4.6):
      - In FromData in pkg/detectors/docker/docker_auth_config.go, replace the common.SaneHttpClient() fallback with detectors.DetectorHttpClientWithNoLocalAddresses (pkg/detectors/http.go:40-45), and remove the pkg/common import, whose only use is that fallback; keep the injected s.client path.
      - Behaviour change: the new fallback follows no redirects and uses the shared client's detector timeout, 10 s by default (pkg/detectors/http.go:25) or the --detector-timeout value, instead of 5 s (pkg/common/http.go:245), so a registry that answers /v2/ with a redirect now yields a verification error, as it already does for the other detectors on the shared client.
    - Added hardening beyond the planned change, to close the residuals it leaves:
      - The residuals, and why the fallback needs its own transport: detectors.DetectorHttpClientWithNoLocalAddresses (pkg/detectors/http.go:40-45) checks the resolved addresses (pkg/detectors/http.go:142-149) but then dials the hostname, which is resolved again (pkg/detectors/http.go:151), so a registry name that answers a public address to the check and a local one to the dial (DNS rebinding) passes it; and when a request goes through a proxy taken from the environment (pkg/detectors/http.go:89), the check sees the proxy's address, not the registry's.
      - Instead of the shared client itself, make the fallback a package-level, Docker-guarded copy of it. Make the copy once, on first use through sync.Once, so it is taken after the CLI has applied any --detector-timeout override (main.go:602-605, pkg/detectors/http.go:50-57). Copy the value of detectors.DetectorHttpClientWithNoLocalAddresses, as detectors.NewClientWithDedup copies a client (pkg/detectors/http.go:264-271); the copy keeps the shared client's Timeout and its no-redirect CheckRedirect (pkg/detectors/http.go:64-70). Set only the copy's Transport, to the Transport of detectors.NewDetectorHttpClient(detectors.WithTransport(detectors.NewDetectorTransport(t))), where t is the Docker transport below, so the copy keeps the shared client's User-Agent and request-instrumentation wrappers (pkg/detectors/http.go:76-79, pkg/detectors/http.go:180) without importing pkg/common. Never assign to the shared client or to its transport.
      - Build t through an unexported constructor in the same file that takes a lookup function, a dial function and a proxy selector, so tests can substitute each one. Production passes net.DefaultResolver.LookupIPAddr, the DialContext of a net.Dialer with the 2 s timeout and 5 s keep-alive of pkg/detectors/http.go:81-84, and http.ProxyFromEnvironment. Copy MaxIdleConns, MaxIdleConnsPerHost, IdleConnTimeout, TLSHandshakeTimeout and ExpectContinueTimeout from pkg/detectors/http.go:91-95.
      - The DialContext of t resolves the registry host once. It returns detectors.ErrNoLocalIP (pkg/detectors/http.go:109) when the lookup returns no address or when any answer is a loopback, link-local unicast or multicast, private or unspecified address, the classes isLocalIP uses (pkg/detectors/http.go:101-107). Otherwise it dials the checked addresses in the order returned, stopping at the first that connects, and never resolves the name again. The TLS handshake still verifies the certificate against the registry host name, because http.Transport takes the server name from the request URL, not from the dialled address.
      - The Proxy function of t returns an error whenever http.ProxyFromEnvironment selects a proxy for the request, so the request is aborted before any connection: a proxy resolves the registry name itself, and the scanner cannot check the address the proxy reaches.
      - Behaviour change that comes from the proxy refusal: with HTTP_PROXY or HTTPS_PROXY set and the registry not excluded by NO_PROXY, Docker verification now returns a verification error instead of going through the proxy. Under the planned change alone, only a proxy at a local address is refused, because the dial guard checks the proxy's address (pkg/detectors/http.go:142-149).
    - Result: the scanner connects only to an address it checked, so neither DNS rebinding nor an environment proxy lets content-supplied registries or realms reach loopback, link-local metadata, private or unspecified addresses. Residual: the transport refuses only the classes isLocalIP uses (pkg/detectors/http.go:101-107), so internal addresses outside them stay reachable, for example 100.64.0.0/10, the shared address space, which includes the Alibaba Cloud metadata address 100.100.100.200. The planned change and every detector on the shared client have the same gap. Extending the refused classes belongs in isLocalIP, which serves every detector and stays untouched, so that decision is left to the project owner. Requests to public hosts named in the scanned content remain, as for every verifying detector, and carry only the credential the same content supplied.
...
- What security tests should verify the fixes?
  - Change A, offline unit tests in pkg/analyzer/analyzers/ngrok/:
    - Origin check: call fetchResources for four URLs: an http://127.0.0.1:PORT URL served by httptest, http://169.254.169.254/, another https host and http://api.ngrok.com/. For the httptest URL, pass the server's own client. For the other three, pass an http.Client whose Transport is a fail-closed recording http.RoundTripper: it records every request it receives and returns an error without opening a connection. Assert that each call returns an error before anything is sent, that the recording transport records zero calls, and that the httptest server records no request, so a missing or regressed origin check fails the test without contacting any address outside the test process.
    - Redirect check (added hardening): build an http.Client with the redirect option AnalyzePermissions installs and a recording http.RoundTripper that answers the on-origin request https://api.ngrok.com/endpoints with a 302 whose Location is an httptest server's http://127.0.0.1:PORT/ URL, forwards a request for that loopback address to the server, and fails closed, without opening a connection, for any other address. Call fetchResources with the on-origin URL and a synthetic test key. Assert that it returns the origin error itself, that the error message contains neither the key nor the Location URL, that the transport recorded only the on-origin request, and that the httptest server recorded no request. Repeat with a Location on https://api.ngrok.com that the transport answers with a 200 response whose body is a JSON page with no resources, for example {}, and assert that the call succeeds, so a same-origin redirect still works.
  - Change B, offline unit tests in pkg/detectors/docker/:
    - Loopback: a Scanner with no injected client runs FromData with verification on, for content whose auths key is the httptest server's http://127.0.0.1:PORT address; the result is unverified with a verification error, and the server records no request.
    - Rebinding (added hardening): build the transport through its constructor with a lookup stub that returns 203.0.113.10, a documentation address the check treats as public, on its first call and 127.0.0.1 on every later call, and a fail-closed recording dial stub. Send a GET for https://registry.test/v2/ on a client over that transport. Assert that the lookup ran once, that the dial stub received only 203.0.113.10:443, never a host name or 127.0.0.1, and that the request fails without opening a connection.
    - Mixed answers (added hardening): with a lookup stub that returns 203.0.113.10 and 127.0.0.1 together, the request fails with an error that errors.Is matches to detectors.ErrNoLocalIP, and the dial stub is never called.
    - Proxy (added hardening): with a proxy selector stub that returns the URL of an httptest server standing in for a proxy, the request fails, the proxy server records no request and no CONNECT, and neither the lookup stub nor the dial stub is called.
