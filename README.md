...
- **The suite collects sixty-two tests rather than thirteen.** All thirteen originally named tests are present, and they are exactly the set that remains once the later tests are deselected. The other forty-nine were added afterwards, each pinning one fix: worker finalization and its terminal update, the compensation applied when a worker thread cannot start, the two stderr-drain outcomes, finding visibility while a scan is still running, a flag-shaped target kept positional, credential redaction for every malformed spelling of a target and in every sink it reaches, the escaping of control characters in the logged command line, the 422 for a body carrying a lone UTF-16 surrogate, and the rejection of a target past the length bound.

Every test runs by default. The forty-nine later tests carry a `regression` marker, so either inventory can be listed on its own: the first command below prints `12` and the second `13`, and the last two report 13 and 49 collected tests.
...
One further acceptance is a dependency pin rather than a shape.

- **`starlette` is pinned explicitly as `starlette==0.46.2` in `backend/requirements.txt`, where the original design left it to FastAPI's own resolution as an unpinned transitive.** Left unpinned, a host that already carries an older starlette from inside the range FastAPI accepts keeps it: in a virtual environment pre-seeded with `starlette==0.41.0`, `pip install -r backend/requirements.txt` finished successfully and left 0.41.0 installed, with `pip check` reporting no broken requirements, because 0.41.0 satisfies FastAPI's bound. The explicit pin makes the installed version — and so the audit result — identical on every host. It does not remove an advisory: `0.46.2` is the ceiling of the `>=0.40.0,<0.47.0` range `fastapi 0.115.14` accepts, and all fourteen releases inside that range carry the same seven advisories, so the pin hardens installation rather than clearing anything. Those seven are PYSEC-2026-161 (CVE-2026-48710), PYSEC-2026-248 (CVE-2026-54282), PYSEC-2026-249 (CVE-2026-54283), PYSEC-2026-1941 (CVE-2025-54121), PYSEC-2026-1942 (CVE-2025-62727), PYSEC-2026-2280 (CVE-2026-48817) and PYSEC-2026-2281 (CVE-2026-48818). The lowest fix version among them is starlette 0.47.2, and `fastapi~=0.115.0` together with `starlette>=0.47.2` gives pip a `ResolutionImpossible`, so `pip-audit -r backend/requirements.txt` still reports all seven and still exits non-zero — the audit is not clean. FastAPI 0.115 is the pinned stack of this prototype, so the seven are accepted here rather than cleared by moving off it, and none of them is reachable from this backend: it mounts no `StaticFiles`, returns no `FileResponse`, subclasses no `HTTPEndpoint`, never calls `request.form()` (the only body parser is JSON through pydantic), never reads `request.url`, and applies no path-based authorization — its only middleware is `GZipMiddleware`. Revisit when the stack pin can move: raising the FastAPI pin to 0.116 or above admits starlette 0.47.2 and is the step that clears these advisories, and because that changes the pinned stack it needs the same review the pin itself had, followed by a re-check of the four routes.
